...
const TableProduct = ({cart}) => {
  ...
  <tbody>
    {cart?.items?.map((product)=>(
      ...
      <td className='bg-dark text-light'><span className="material-symbols-outlined"onClick={()=>addToCart(product?.productId,product.title,product.price/product.qty,1,product.imgSrc)} >
add_circle
</span></td>
      <td className='bg-dark text-light'><span className="material-symbols-outlined"  onClick={()=>decreaseQty(product?.productId,1)}>
do_not_disturb_on
</span></td>
      <td className='bg-dark text-light'><span className="material-symbols-outlined" onClick={()=>{if(confirm('Are you sure, want to remove from Cart'))removeFromCart(product?.productId)}}>
delete
</span></td>
    </tr>))}
    <tr>
